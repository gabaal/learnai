function ChapterTopicList({course}) {
  ...
  return (
    <div><h2 className="font-bold text-3xl mt-10">Chapters & Topics</h2>
    <div className="flex flex-col items-center justify-center mt-10">
       {courseLayout?.chapter?.map((chapter, index) => (
                ...
                {chapter?.topics?.map((topic, index) => (
                    <div className="flex flex-col items-center" key={index}>
                        ...
                        <div className="flex items-center gap-5">
                            ...
                            <h2 className="text-center rounded-full bg-gray-300 px-6 text-gray-500 p-4"></h2>
                            <span className={`${index % 2 != 0 && 'text-transparent'} max-w-xs`}>{topic}</span>
                        </div>
{index == chapter?.topics?.length - 1 && <div className="h-10">{}</div>}
